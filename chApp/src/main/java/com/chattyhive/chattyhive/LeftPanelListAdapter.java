...
public class LeftPanelListAdapter extends BaseAdapter {
    ...
    private void CaptureChats() {
        ...
        // TODO: list.addAll(Controller.GetRunningController().getUser("").getChatSubscriptionsList().);
    ...
    private void CaptureFriends() {
        ...
        User me = ((Main)context).controller.getUser("");
        if (me != null)
            ...
            ((ChatViewHolder) holder).chatHiveImage.setImageResource(R.drawable.default_hive_image);
                    ...
                    switch (((Chat) item).getChatType()) {
                        case PRIVATE_HIVEMATE:
                            ...
                            try {
                                ((PrivateHivemateChat) item).getHive().getHiveImage().OnImageLoaded.add(new EventHandler<EventArgs>(holder, "loadHiveImage", EventArgs.class));
                                ((PrivateHivemateChat) item).getHive().getHiveImage().loadImage(Image.ImageSize.small, 0);
                                ((ChatViewHolder) holder).hiveName = context.getResources().getString(R.string.hivename_identifier_character).concat(((PrivateHivemateChat) item).getHive().getName());
                            } catch (Exception e) {
                            }
                            for (User user : ((PrivateHivemateChat) item).getUsers())
                                if (!user.isMe()) {
                        ...
                        case PRIVATE_GROUP_HIVEMATE:
                           ...
                           /* if ((((Chat) item).getName() != null) && (!((Chat) item).getName().isEmpty()))
                                ...
                                for (User user : ((Chat) item).getUsers())
                                    if (!user.isMe()) {
                                        if ((user.getUserPublicProfile() != null) && (user.getUserPublicProfile().getShowingName() != null))
                        ...
                        case PUBLIC:
                            ...
                            try {
                                ((PublicHiveChat) item).getHive().getHiveImage().OnImageLoaded.add(new EventHandler<EventArgs>(holder, "loadChatImage", EventArgs.class));
                                ((PublicHiveChat) item).getHive().getHiveImage().loadImage(Image.ImageSize.medium, 0);
                            } catch (Exception e) {
                            }
                            if (((PublicHiveChat) item).getHive() != null)
                                GroupName = context.getResources().getString(R.string.hivename_identifier_character).concat(((PublicHiveChat) item).getHive().getName());
    ...
    private class FriendViewHolder extends ViewHolder {
        ...
        private void updateData() {
            if ((this.cardFriend.getUserPrivateProfile() == null) || (this.cardFriend.getUserPrivateProfile().getLoadedProfileLevel().ordinal() < ProfileLevel.Basic.ordinal())) {
                ...
                this.cardFriend.loadProfile(ProfileType.PRIVATE, ProfileLevel.Basic,Controller.GetRunningController().getAccountID(null));
                return;
            }
            if ((this.cardFriend.getUserPublicProfile() == null) || (this.cardFriend.getUserPublicProfile().getLoadedProfileLevel().ordinal() < ProfileLevel.Basic.ordinal())) {
                this.cardFriend.UserLoaded.add(new EventHandler<EventArgs>(this, "onUserLoaded", EventArgs.class));
                this.cardFriend.loadProfile(ProfileType.PUBLIC, ProfileLevel.Basic,Controller.GetRunningController().getAccountID(null));
                return;
            }
